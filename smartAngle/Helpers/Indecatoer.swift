...
public class SKActivityIndicator: NSObject {
    ...
    //init(red  : 0.0, green  : 0.0, blue  : 0.0, alpha  : 0.2)
    ...
    private override init() {
        ...
        if let windowObj = delegate.window ,let _window = windowObj {
            window = _window
        } else if let windowObj = UIApplication.shared.keyWindow{
            window = windowObj
        }else if let windowObj = UIApplication.shared.windows.first {
            window = windowObj
        }
        statusLabel = nil
        backgroundView = nil
